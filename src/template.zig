...
fn middlewareHandler(ptr: ?*anyopaque, _: *ziez.Request, res: *ziez.Response, next: *ziez.Next) void {
    res.template_engine = ptr;
    next.call();
}

/// Returns a Middleware that injects the template engine pointer into every response.
/// The caller owns the TemplateEngine; it is NOT freed at app.deinit().
pub fn asMiddleware(engine: *TemplateEngine) ziez.Middleware {
    return .{
        .ptr = engine,
